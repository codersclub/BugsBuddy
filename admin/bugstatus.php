...
function getbugstatus() {
  ...
  if (!empty($_GET['id'])) {
    if (is_numeric($_GET['id']) && intval($_GET['id']) != 1) {
      ...
      $returnValue .= '<a href="index.php?'.((isset($_GET['js'])&&$_GET['js']=='yes')?'js=yes':'js=no').'&page=bugstatus">'. lang('back') .'</a>';
    } else {
      ...
      if (is_numeric($_POST['delthis'])) {
        if (intval($_POST['delthis']) != 1) {
          ...
          if ($bugnamecount > 0) {
            ...
            $returnValue .= '<a href="index.php?'.((isset($_GET['js'])&&$_GET['js']=='yes')?'js=yes':'js=no').'&page=bugstatus&id=' . $_POST['delthis'] . '">' . lang('yes') . '</a>&nbsp;';
            $returnValue .= '&nbsp;<a href="index.php?'.((isset($_GET['js'])&&$_GET['js']=='yes')?'js=yes':'js=no').'&page=bugstatus">' . lang('no') . '</a>';
          } else {
        ...
        $returnValue .= '<div><a href="index.php?'.((isset($_GET['js'])&&$_GET['js']=='yes')?'js=yes':'js=no').'&page=bugstatus">'. lang('back') .'</a></div>';
